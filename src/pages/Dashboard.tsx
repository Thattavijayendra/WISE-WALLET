...
const Dashboard: React.FC = () => {
  ...
  // Automatically trigger notification permission prompt on Dashboard mount
  useEffect(() => {
  ...
  useEffect(() => {
    if (budget && expenses) {
      ...
      const thisMonthExpenses = expenses.filter(expense => expense.date >= firstDayOfMonth);
      ...
      const todaysExpenses = expenses.filter(expense => expense.date >= todayStart);
